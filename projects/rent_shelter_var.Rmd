...
serial_test
```
The Portmanteau test was applied to formally test for residual autocorrelation. As reported in **Table 6**, the test yields a p-value of **`r round(p_val,2)`**, which is greater than 0.05. Consequently, we fail to reject the null hypothesis of no serial correlation, supporting the adequacy of the model specification.
...
Results reported in **Table 7** show that changes in median rent Granger-cause changes in shelter population, with a p-value of **`r round(p_rent_to_shelter,2)`**. Similarly, changes in shelter population Granger-cause changes in median rent, with a p-value of **`r round(p_shelter_to_rent,2)`**.
...
```{r shelter-rent-subfigures, echo=FALSE, fig.align='center', fig.cap="Shelter Population and Median Rent Over Time", fig.subcap=c("Shelter Population", "Median Rent Asked"), message=FALSE, warning=FALSE, out.height="40%", out.width="100%"}
par(mfrow = c(1, 2))
...
```{r shelter-rent-growth-subfigures, echo=FALSE, fig.align='center', fig.cap="Growth Rates of Shelter Population and Median Rent", fig.subcap=c("Growth Rate of Shelter Population", "Growth Rate of Median Rent"), message=FALSE, warning=FALSE, out.height="40%", out.width="100%"}
...
```{r shelter-rent-resid-subfigures, echo=FALSE, fig.align='center', fig.cap="Residuals Plots of VAR(4) Model", fig.subcap=c("Change in Shelter (Percentage Change)", "Change in Rent (Percentage Change)"), message=FALSE, warning=FALSE, out.height="40%", out.width="100%"}
...
```{r acf-pacf-residuals, echo=FALSE, fig.align='center', fig.cap="ACF and PACF of Residuals for Shelter and Rent", fig.subcap=c("ACF: Shelter", "PACF: Shelter", "ACF: Rent", "PACF: Rent"), message=FALSE, warning=FALSE, out.height="80%", out.width="100%"}
...
```{r irf-plot, echo=FALSE, fig.cap="Impulse Response Functions from VAR(4)", message=FALSE, warning=FALSE, out.width="100%"}


par(cex.axis = 0.7,  
    cex.lab  = 0.8)  

irf_all <- irf(var_model,impulse  = c("dln_shelter", "dln_rent"), response = c("dln_shelter", "dln_rent"), n.ahead  = 12, boot     = TRUE,ci= 0.95)
...
plot_shelter <- ggplot(shelter_all, aes(x = month, y = shelter, 
  ...
  geom_line(size = 1) +
  labs(
    title = "Shelter Population: Actual and Forecast",
    x = "Month",
...
plot_rent <- ggplot(rent_all, aes(x = month, y = rent, 
  ...
  geom_line(size = 1) +
  labs(
    title = "Median Rent: Actual and Forecast",
    x = "Month",
    y = "Median Rent (USD)"
  ) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
adf_results %>% 
  kable("latex", booktabs = TRUE, digits = 3, caption = "ADF Test Results for Shelter Population and Median Rent") %>%
...
adf_ln_results %>%
  kable("latex", booktabs = TRUE, digits = 3,
        caption = "ADF Test Results (Log Variables) with Drift and Trend") %>%
  kable_styling(latex_options = c("hold_position", "striped"))
```
...
adf_dlog_results %>%
  kable("latex", booktabs = TRUE, digits = 3,
        caption = "ADF Test Results for Differenced Log Variables") %>%
  kable_styling(latex_options = c("hold_position", "striped"))
```
...
johansen_results %>%
  kable("latex", booktabs = TRUE, digits = 2,
        caption = "Johansen Cointegration Test Results (Maximum Eigenvalue Statistic)") %>%
  kable_styling(latex_options = c("hold_position", "striped"))
```
...
lag_selection %>%
  kable("latex", booktabs = TRUE, digits = 4,
        caption = "Lag Length Selection Criteria (AIC, HQ, SC(BIC))") %>%
  kable_styling(latex_options = c("hold_position", "striped"))
```
...
var_results %>%
  kable("latex", booktabs = TRUE, digits = 4,
        caption = "Estimation Results for VAR(4) Model") %>%
  kable_styling(latex_options = c("hold_position", "striped"))
```
...
serial_tbl %>% kable("latex", booktabs = TRUE, digits = 4, caption = "Portmanteau Serial Correlation Test (12 Lags)") %>%
...
forecast_manual %>%
  kable(
    ...
    booktabs = TRUE,
    digits = 4,
    caption = "12-Month Forecasts: Percentage Changes and Levels"
  ) %>%
  kable_styling(latex_options = c("hold_position"))
```
